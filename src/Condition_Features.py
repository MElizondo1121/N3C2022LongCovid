...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.60f2eb0d-7b8c-466f-9ae8-66105e882875"),
    Conditions=Input(rid="ri.foundry.main.dataset.95a7b607-2c7b-4f72-99ab-9dc8747b4eae")
)
def Condition_Features(Conditions):
    df = Conditions
...
'Effusion of joint of left elbow']
    ...
    df = df.withColumn('LossOfTaste_Cond', when(col('condition_concept_name').isin(loss), col('condition_duration')))
    df = df.withColumn('Allergic_rhinitis_Cond', when(col('condition_concept_name').isin(seasonal), col('condition_duration')))
    df = df.withColumn('Bypass_graft_Cond', when(col('condition_concept_name').isin(bypass), col('condition_duration')))
    df = df.withColumn('Cough_Cond', when(col('condition_concept_name').isin(cough), col('condition_duration')))
    df = df.withColumn('Fever_Cond', when(col('condition_concept_name').isin(fever),  col('condition_duration')))
    df = df.withColumn('Fatigue_Cond', when(col('condition_concept_name').isin(fatigue), col('condition_duration')))
    df = df.withColumn('Renal_Cond', when(col('condition_concept_name').isin(renal), col('condition_duration')))
    df = df.withColumn('Obesity_Cond', when(col('condition_concept_name').isin(obesity), col('condition_duration')))
    df = df.withColumn('Covid_Cond', when(col('condition_concept_name').isin(covid), col('condition_duration')))
    df = df.withColumn('Brain_injury_Cond', when(col('condition_concept_name').isin(brain_injury), col('condition_duration')))
    df = df.withColumn('Deformity_foot_Cond', when(col('condition_concept_name').isin(deformity_foot), col('condition_duration')))
    df = df.withColumn('Respiratory_fail_Cond', when(col('condition_concept_name').isin(respiratory_failure), col('condition_duration')))
    df = df.withColumn('Oltagia_Cond', when(col('condition_concept_name').isin(oltagia), col('condition_duration')))
    df = df.withColumn('Venticular_Cond', when(col('condition_concept_name').isin(venticular), col('condition_duration')))
    df = df.withColumn('Elevation_Cond', when(col('condition_concept_name').isin(elevation), col('condition_duration')))
    df = df.withColumn('Trial_fib_Cond', when(col('condition_concept_name').isin(trial_fib), col('condition_duration')))
    df = df.withColumn('Disorders_Cond', when(col('condition_concept_name').isin(disorders), col('condition_duration')))
    df = df.withColumn('Effusion_Cond', when(col('condition_concept_name').isin(effusion_of_joint), col('condition_duration')))
    df = df.withColumn('Hernia_Cond', when(col('condition_concept_name').isin(hernia), col('condition_duration')))
    df = df.withColumn('Nutricional_def_Cond', when(col('condition_concept_name').isin(nutricional_def), col('condition_duration')))
    df = df.withColumn('Pain_limb_Cond', when(col('condition_concept_name').isin(pain_in_limb), col('condition_duration')))
    df = df.withColumn('Pain_hand_Cond', when(col('condition_concept_name').isin(pain_in_hand), col('condition_duration')))
    df = df.withColumn('Cyst_Cond', when(col('condition_concept_name').isin(cyst), col('condition_duration')))

    df = df.withColumn('Other_Cond', when((col('condition_concept_name').isin(cough) ==False) & (col('condition_concept_name').isin(fever) ==False)& (col('condition_concept_name').isin(fatigue) ==False)&  (col('condition_concept_name').isin(renal) ==False)& (col('condition_concept_name').isin(obesity) ==False)&(col('condition_concept_name').isin(covid) ==False)& (col('condition_concept_name').isin(brain_injury) ==False)&(col('condition_concept_name').isin(deformity_foot) == False)&(col('condition_concept_name').isin(respiratory_failure) == False)&(col('condition_concept_name').isin(oltagia) == False)&(col('condition_concept_name').isin(venticular) == False)&(col('condition_concept_name').isin(elevation) == False)&(col('condition_concept_name').isin(trial_fib) == False)&(col('condition_concept_name').isin(disorders) == False)&(col('condition_concept_name').isin(effusion_of_joint) == False)&(col('condition_concept_name').isin(hernia) == False)&(col('condition_concept_name').isin(nutricional_def) == False)&(col('condition_concept_name').isin(pain_in_limb) == False)&(col('condition_concept_name').isin(pain_in_hand) == False)&(col('condition_concept_name').isin(cyst) == False)&(col('condition_concept_name').isin(loss) == False)&(col('condition_concept_name').isin(seasonal) == False)&(col('condition_concept_name').isin(bypass) == False),col('condition_duration')))
